...
export async function getStatusV1(request, response) {
	...
	try {
		// Get Cache status
		let	[upToDate, error] =
					await request.app.locals.documentStorage.cacheGetStatus(documentStorageID, name);
		if (upToDate)
			// Success
			response
					.status(200)
...
//----------------------------------------------------------------------------------------------------------------------
// MARK: Get Values
export async function getValuesV1(request, response) {
	// Setup
	let	documentStorageID = decodeURIComponent(request.params.documentStorageID);
	let	name = decodeURIComponent(request.params.name);
	...
	try {
		...
		let	[upToDate, results, error] =
					await request.app.locals.documentStorage.cacheGetValues(documentStorageID, name, valueNames,
							documentIDs);
		if (upToDate)
			// Success
			response
