...
extension MDSDocument {
	...
	//------------------------------------------------------------------------------------------------------------------
	public func data(for property :String) -> Data? {
		// Retrieve Base64-encoded string
		guard let string = self.documentStorage.value(for: property, in: self) as? String else { return nil }
	...
	public func nsError(for property :String) -> NSError? {
		// Retrieve info
		guard let info = map(for: property), let domain = info["NSDomain"] as? String,
				let code = info["NSCode"] as? Int else { return nil }
...
public class MDSDocumentInstance : Hashable, MDSDocument {
	...
	// MARK: Properties
	class	public	var documentType: String { fatalError("Trying to get documentType from MDSDocumentInstance") }
